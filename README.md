...
| `/bearings`        | Generate a concise four-section chat digest from bounded local fleet state, including registered remote-home ledgers; use `/bearings file` to also replace today's dated report in `data/`, `/bearings lavish` to build and arm an interactive Lavish fleet board, and add `include PRs` for live PR enrichment |
| `/updatefirstmate` | Fast-forward the running firstmate and its secondmates to the latest from origin, persist and restart every live mate successfully left on the target commit - including already-current homes - then re-read instructions and nudge secondmates, with an honest re-read nudge when restart cannot be proven |
