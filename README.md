...
- `skills/` - public, installer-facing skills meant to be installed standalone into any project, independent of firstmate.
  ...
  `skills/bws` and `skills/postgres-admin` are each the single procedure owner for their topic; firstmate's `.agents/skills/bws` and `.agents/skills/postgres-admin` stubs only route loaders to them.
