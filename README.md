...
- `skills/` - public, installer-facing skills meant to be installed standalone into any project, independent of firstmate.
  ...
  `skills/bws`, `skills/kubectl-ops`, and `skills/postgres-admin` are each the single procedure owner for their domain; firstmate's `.agents/skills/bws`, `.agents/skills/kubectl-ops`, and `.agents/skills/postgres-admin` stubs only route loaders to them.
